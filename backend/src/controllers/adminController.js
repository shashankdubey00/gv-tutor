...
/* ---------------- CREATE & POST TUTOR REQUEST FROM POSTER ---------------- */
export const createPosterTutorRequest = async (req, res) => {
  try {
    const { classLevel, board, subjects, time, teacher, location } = req.body;

    if (!classLevel || !subjects || !time || !location) {
      return res.status(400).json({
        success: false,
        message: "Class, subjects, time, and location are required",
      });
    }
    ...
    const teacherText = String(teacher || "").toLowerCase();
    let preferredTutorGender = "any";
    if (teacherText.includes("female")) preferredTutorGender = "female";
    if (teacherText.includes("male")) preferredTutorGender = "male";

    const teacherExperienceMatch = String(teacher || "").match(/\d+/);
    const teacherExperience = teacherExperienceMatch
      ? Math.max(0, Math.min(50, parseInt(teacherExperienceMatch[0], 10)))
      : 0;

    const request = await TutorRequest.create({
      parentName: "Admin Poster Post",
      parentEmail: "",
      parentPhone: "NA",
      studentGrade: String(classLevel).trim(),
      subjects: subjectList,
      preferredLocation: String(location).trim(),
      preferredTiming: String(time).trim(),
      frequency: "weekly",
      budget: "Discuss with parent",
      preferredTutorGender,
      teacherExperience,
      additionalRequirements: [
        board ? `Board: ${String(board).trim()}` : "",
        teacher ? `Teacher Preference: ${String(teacher).trim()}` : "",
        "Source: Poster Post",
      ]
        .filter(Boolean)
        .join(" | "),
      status: "posted",
      // Keep private parent contact placeholders hidden for tutors.
      fieldVisibility: {
        parentName: false,
        parentEmail: false,
        parentPhone: false,
    ...
    try {
      ...
      console.error("Poster job notification failed:", notifError);
    }

    return res.status(201).json({
      success: true,
      message: "Poster job posted successfully",
      request,
    });
  } catch (error) {
    console.error("Create poster tutor request error:", error);
    return res.status(500).json({
      success: false,
      message: "Internal server error",
    });
